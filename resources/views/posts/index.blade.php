...
<body>
    ...
    <div class="menu">
        <ul id="list">
            ...
            <a class="tdecor" href="{{ route('posts.index') }}"><li id="list-item6" class="list-item">نمایش پست ها</li></a>
        </ul>
    </div>
        ...
        @if($posts)
            @foreach($posts as $post)
                <div id="indiv1" class="indiv">
                    ...
                    <div class="cardin">
                        <img src="{{ $post->photo ? asset($post->photo->path) : asset('photo/noimage.png') }}" width="245px" height="220px">

                        <div class="cardinp">
                            <p id="p1">کاربر : <a class="ast" href="{{ route('posts.edit', $post -> id) }}" id="aname">  {{ $post -> user -> name }}</a></p>
                            <p id="p2">دسته :   {{ $post -> category ? $post -> category -> name : 'دسته ای وجود ندارد' }}</p>
                            <p id="p3">عنوان :   {{ $post -> title ? $post -> title : "کاربر پستی ندارد" }}</p>
                            <p id="p4">متن :   {{ $post -> body ? $post -> body : "متنی ثبت نشده است" }}</p>
                           
                            <p id="p5" class="datess">{{ $post -> created_at ? $post -> created_at->diffForHumans() : "" }} : ساخت</p>
                            <p id="p5" class="datess">{{ $post -> updated_at ?  $post -> updated_at-> diffForHumans() : "" }} : به روز رسانی</p>
                        </div>
                    </div>
                </div>
            @endforeach
